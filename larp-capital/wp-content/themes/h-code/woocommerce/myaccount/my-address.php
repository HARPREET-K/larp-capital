...
<div class="col-md-12 margin-bottom-10px">
	<h2 class="black-text font-weight-600 text-uppercase title-small"><?php echo esc_attr($page_title); ?></h2>
...
<?php foreach ( $get_addresses as $name => $title ) : ?>
	...
	<div class="u-column<?php echo ( ( $col = $col * -1 ) < 0 ) ? 1 : 2; ?> col-<?php echo ( ( $oldcol = $oldcol * -1 ) < 0 ) ? 1 : 2; ?> woocommerce-Address address col-sm-6 col-xs-12">
		<div class="woocommerce-Address-title title">
			<h3 class="black-text font-weight-600 text-uppercase text-large margin-bottom-10px"><?php echo esc_attr($title); ?></h3>
		...
		<address>
			<?php
				...
				if ( ! $formatted_address )
					...
					echo sprintf( __('%s','H-Code'), $formatted_address );
			?>
		</address>
	</div>
