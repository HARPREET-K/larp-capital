	  ...
	  ============================================================================*/
		public function __construct() {
			$this->opt = array(
				...
				//option of breadcrumb title hide & show,	
					'breadcrumb_title_hide'    => false,
			...
			************************************************* */
			...
			case 'page':
				...
				if( strtolower($bcn_pagetitle) != strtolower($this->opt['title_home']) ) {  // Check if we are not on home
					...
					if($this->opt['breadcrumb_title_hide'] === true){
			...
			case 'singleportfolio':
				...
				$hcode_enable_portfolio_text_breadcrumb = (isset($hcode_options['hcode_enable_portfolio_text_breadcrumb'])) ? $hcode_options['hcode_enable_portfolio_text_breadcrumb'] : '';
				$hcode_portfolio_breadcrumb_text = (isset($hcode_options['hcode_portfolio_breadcrumb_text']) && !empty($hcode_options['hcode_portfolio_breadcrumb_text'])) ? $hcode_options['hcode_portfolio_breadcrumb_text'] : 'Portfolio';
				$hcode_portfolio_text_url = (isset($hcode_options['hcode_portfolio_text_url']) && !empty($hcode_options['hcode_portfolio_text_url'])) ? '<a href="'.$hcode_options['hcode_portfolio_text_url'].'">'.$hcode_portfolio_breadcrumb_text.'</a>' : $hcode_portfolio_breadcrumb_text;
				...
				if(get_post_type() == 'portfolio'){
					if( $hcode_enable_portfolio_text_breadcrumb == 1 ){
						$result_array['middle'] .= '<li>'.$hcode_portfolio_text_url.'</li>';
					}
			...
			case 'portfolio':
				...
				$cat_breadcrumbs = '';
				while ($parent_id) {
					$category   = get_term($parent_id);
					
					$cat_breadcrumbs = '<li><a href="' . get_term_link($category->term_id) . '" title="' . $this->opt['urltitle_prefix'] . $category->name . $this->opt['urltitle_suffix'] . '">' . $category->name . '</a></li>' . $this->opt['separator'] . $cat_breadcrumbs;
					$parent_id  = $category->category_parent;
				}
